...
interface NetworkProfileFormProps {
    ...
    networkType?: string; // Add networkType prop
}

const NetworkProfileForm: React.FC<NetworkProfileFormProps> = ({ profile, onChange, title, distributedSwitches = [], networkType }) => {
            ...
            <div className="grid">
                <div className="col-12 md:col-6">
                    <div className="field">
                        ...
                        <InputText value={profile.network} onChange={(e) => onChange({ ...profile, network: e.target.value })} className="w-full" />
                    </div>
                </div>
                <div className="col-12 md:col-6">
                    <div className="field">
                        <label className="font-bold">Gateway</label>
                        <InputText value={profile.gateway} onChange={(e) => onChange({ ...profile, gateway: e.target.value })} className="w-full" />
                    </div>
                </div>
                <div className="col-12 md:col-6">
                    <div className="field">
                        <label className="font-bold">Prefix</label>
                        <InputNumber value={profile.prefix} onValueChange={(e) => onChange({ ...profile, prefix: e.value?.toString() || '' })} className="w-full" min={0} max={32} placeholder="Enter network prefix (0-32)" />
                    </div>
                </div>
                <div className="col-12 md:col-6">
                    <div className="field">
                        <label className="font-bold">IP Addresses</label>
                        <Chips value={profile.ip} onChange={(e) => onChange({ ...profile, ip: e.value })} className="w-full" placeholder="Enter IP and press Enter" allowDuplicate={false} separator="," />
                    </div>
                </div>
                {networkType !== 'vmotion' && networkType !== 'replication' && (
                    <div className="col-12 md:col-6">
                        <div className="field">
                            <div className="text-gray-500">
                                ...
                                <ul className="list-none p-0 m-0">
                                    <li>• 1 IP for IX</li>
                                    <li>
                                        • {totalNECount} IP{totalNECount > 1 ? 's' : ''} for NE ({totalHACount} HA pairs, {totalNECount - totalHACount} standalone)
                                    </li>
                    ...
                    <div className="col-12 md:col-6">
                        <div className="field">
                            <div className="text-gray-500">
                                ...
                                <ul className="list-none p-0 m-0">
                                    <li>• 1 IP for IX</li>
                                    <li>• Total: 1 required</li>
                                    <li className={profile.ip.length >= 1 ? 'text-green-500' : 'text-red-500'}>
                                        • {profile.ip.length} IP{profile.ip.length !== 1 ? 's' : ''} provided
                                    </li>
                                </ul>
                            </div>
